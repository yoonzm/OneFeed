...
* **文章评论代理**：Article Detail 的评论采用“静态能力描述 + 按需评论快照”，Adapter 负责驱动原站局部评论、全部评论弹层与增量加载，Renderer 点击评论后统一打开 OneFeed 弹窗，不在文章底部插入评论，也不直接渲染平台 DOM。首个适配目标为知乎回答详情，具体契约与验收标准见[文章详情评论功能设计](Comment_Feature_Design.md)。
