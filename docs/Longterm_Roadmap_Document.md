...
* **安全回退**：浏览器工具栏图标打开启动中心，启动中心总开关与页面右侧常驻悬浮开关共同切换全局接管状态。悬浮按钮只负责一键暂停或重新开启；暂停后立即卸载接管层并恢复原页面，初始化异常时自动回退，用户可随时重新开启；该状态不会在 Chrome 中禁用扩展本身。
...
#### 6. 阅读界面偏好（已完成）
* **顶部网站目录**：用户可在扩展设置页选择阅读器 Header 展示的网站并调整顺序；偏好只改变 OneFeed 导航，不影响适配器范围。当前正在浏览的平台即使被配置为隐藏，也会临时保留在 Header 中，避免丢失来源与频道上下文。
* **内容图片控制**：信息流列表与详情页分别保存图片显示偏好。关闭后不挂载标准 Gallery、链接预览图和纯视频封面，并隐藏富文本中的内容图片；作者头像、平台标识、行内表情和可播放视频不受影响。详情偏好同时覆盖讨论条目与评论弹层。
* **设置入口分工**：阅读器 Header 最右侧提供设置按钮并打开完整设置页，设置页统一承载平台显隐/排序、图片显示和自定义过滤规则；页面悬浮按钮只负责全局接管开关，工具栏图标继续打开启动中心，不新增重复的 Toolbar Popup。
