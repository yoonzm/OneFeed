...
| 图书发现 | 微信读书公开榜单、分类与检索结果 | 首期复用 `article + previewBlocks` 表达书名、作者、简介、封面、今日阅读与推荐值；不接管阅读器正文或个人数据页面 |
...
| **2.4 公开图书发现（已完成）** | 验证图书榜单、分类频道、检索与滚动加载 | 微信读书 | 仅接管公开榜单、分类及带关键词或作者的图书检索；解析书名、作者、简介、封面、今日阅读与推荐值；阅读器正文、书籍详情、个人书架、笔记与阅读进度保持原站体验 |

Phase 2 前期不以 Adapter 数量为 KPI，而以阅读链路的稳定性为先。当前正式支持微博热门首页、单条微博详情及内容检索，X 首页时间线，小红书发现页、频道与支持评论阅读的笔记详情，Reddit 首页/排序页/社区 Feed，知乎、微信读书公开图书榜单/分类/检索、Hacker News、Linux DO、V2EX，以及 36Kr 资讯频道及文章详情。Schema 理论上能够表达某类内容，不代表已经交付对应平台适配；平台支持状态以 README 为准。
