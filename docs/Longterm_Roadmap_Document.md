...
* **交付顺序**：`0.1.0` 先交付知乎完整链路，后续迭代已通过统一 Adapter 注册契约接入 Twitter/X；单平台首发用于降低同时调试多个动态 DOM 的风险，不改变 Phase 1 的双平台目标。
...
| 阶段 | 目标节点 | 关键 KPI 指标 |
| :--- | :--- | :--- |
| **Phase 1** | M1 - M2 | 完成 MVP 开发，知乎/Twitter 渲染成功率 > 95%，卡片解析延迟 < 100ms。 |
